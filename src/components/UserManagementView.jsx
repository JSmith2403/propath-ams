...
function AthletePermissions({ user, athletes, onAdd, onRemove }) {
  ...
  const toggle = async (athleteId, isAllocated) => {
    setBusy(athleteId);
    try {
      if (isAllocated) await onRemove(user.id, athleteId);
            ...
            return (
              ...
              >
                <input
                  ...
                  className="rounded border-gray-300 disabled:opacity-40 shrink-0"
                ...
                {isBusy && <span className="text-xs text-gray-300 shrink-0">…</span>}
              </label>
            );
          })}
        </div>
        ...
        <div className="flex-1 min-w-0 text-left">
          ...
          {isExternal && (
            <p className="text-xs text-gray-400 mt-0.5">
              {user.allocations.length} athlete{user.allocations.length !== 1 ? 's' : ''} assigned
            </p>
          )}
        </div>
